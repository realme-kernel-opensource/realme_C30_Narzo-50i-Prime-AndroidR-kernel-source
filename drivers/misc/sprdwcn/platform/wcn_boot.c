...
static void marlin_set_bit(enum wcn_sub_sys bit)
{
	marlin_dev->power_state |= 1 << bit;
}

static void marlin_clear_bit(enum wcn_sub_sys bit)
{
	marlin_dev->power_state &= ~(1 << bit);
}
...
static int find_firmware_path(void)
{
	...
	if (pre_len + strlen("gnssmodem") >= 254) {
		pr_err("firmware path is too long\n");
		return -1;
	}
		...
		if (marlin_dev->keep_power_on) {
			if (!flag_reset) {
				if (subsys != WCN_AUTO) {
					...
					marlin_set_bit(WCN_AUTO);
					marlin_clear_bit(subsys);
				}
