...
static void tcpm_fixed_pd_complete(struct tcpm_port *port)
{
...
static int tcpm_fixed_pd_deactivate(struct tcpm_port *port)
{
	int i, ret = 0;
	...
	if (port->nr_snk_pdo <= 0 ||
	    pdo_type(port->snk_pdo[0]) != PDO_TYPE_FIXED ||
	    pdo_fixed_voltage(port->snk_pdo[0]) != 5000) {
		port->nr_snk_pdo = 1;
		port->snk_pdo[0] = PDO_FIXED(5000, 3000, 0);
	...
	if (!wait_for_completion_timeout(&port->fixed_pd_complete,
				msecs_to_jiffies(PD_CTRL_TIMEOUT)))
		ret = -ETIMEDOUT;
...
void tcpm_shutdown(struct tcpm_port *port)
{
	...
	if (port->pps_data.active) {
		...
		ret = tcpm_fixed_pd_deactivate(port);
		if (ret)
