	...
	if (boot_mode_check("androidboot.mode=autotest")) {
		...
		drm_for_each_crtc(crtc, drm) {
			if (!crtc->state->active) {
				...
				dpu = container_of(crtc, struct sprd_dpu, crtc);
				dsi = dpu->dsi;
				is_inited = sprd_dsi_encoder_disable_force(&dsi->encoder);
				bbat_force_off = true; /* For BBAT deep sleep */
				if (is_inited) {
					DRM_INFO("display is already inited\n");
					break;
				} else {
					DRM_INFO("display force power down now\n");
					return 0;
				}
			}
			bbat_force_off = true;
...
static int sprd_drm_pm_resume(struct device *dev)
{
	...
	if (bbat_force_off) {
		DRM_WARN("force power down not need resume!\n");
		return 0;
	}
