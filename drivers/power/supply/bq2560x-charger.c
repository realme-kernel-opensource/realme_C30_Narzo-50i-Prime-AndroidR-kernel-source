...
#define BQ2560X_REG_ITERM_EN_MASK           BIT(7)
#define BQ2560X_REG_ITERM_EN_SHIFT          7
...
#define SGM41512_VENDER_ID             (0x5)
#define SYV6974B_VENDOR_ID             (0x8)
#define BQ2560X_VENDOR_ID_MASK		GENMASK(6, 3)
#define BQ2560X_VENDOR_ID_SHIFT		3
			       ...
			       u8 mask, u8 data)
{
	...
	if (info->chip_id == SGM41512_VENDER_ID) {
		ret_w = bq2560x_write(info, reg, v);
		if (ret_w < 0) {
			dev_info(g_charger_info->dev, "[%s][%d] bq2560x_write failed.\n", __func__, __LINE__);
			return ret_w;
		}

		ret_r = bq2560x_read(info, reg, &v_r);
		if (ret_r < 0) {
			dev_info(g_charger_info->dev, "[%s][%d] bq2560x_read failed.\n", __func__, __LINE__);
			return ret_w;
		}

		if (BQ2560X_REG_B == reg) {
			dev_info(g_charger_info->dev, "[%s][%d]reg0x%d  v_r:0X%x, v_w:0X%x \n", __func__, __LINE__, reg, v_r, v);
			return ret_w;
		}
	
		if (BQ2560X_REG_RESET_MASK == mask) {
			dev_info(g_charger_info->dev, "[%s][%d]reg0x%d  v_r:0X%x, v_w:0X%x \n", __func__, __LINE__, reg, v_r, v);
			return ret_w;
		}

		if ((v & mask) == (v_r & mask)) {
			dev_info(g_charger_info->dev, "[%s][%d]reg0x%d write data and read data is same.\n", __func__, __LINE__, reg);
			return ret_w;
		} else {
			dev_info(g_charger_info->dev, "[%s][%d]reg0x%d write data v_r:0X%xand read data v_w:0X%x is not same.\n", __func__, __LINE__, reg, v_r, v);
...
int bq2560x_set_batfet_dis(struct bq2560x_charger_info *info)
{
	u8 v = 0;
	int ret = 0;

	ret = bq2560x_read(info, BQ2560X_REG_7, &v);
...
static void bq2560x_set_watchdog_timer_status(struct bq2560x_charger_info *info, u8 reg_val)
{
	u8 v;

	if (info->role == BQ2560X_ROLE_MASTER_DEFAULT) {
		bq2560x_update_bits(info, BQ2560X_REG_5,
					   BQ2560X_REG_WATCHDOG_TIMER_MASK,
					   reg_val << BQ2560X_REG_WATCHDOG_TIMER_SHIFT);
		bq2560x_read(info, BQ2560X_REG_5, &v);
		dev_err(info->dev, "master ic set_watchdog_timer 0x%x = 0x%x\n", BQ2560X_REG_5, v);
	} else {
		dev_err(info->dev, "only one charge ic\n");
	}
}
...
static int bq2560x_charger_set_termina_en(struct bq2560x_charger_info *info, int val)
{
	u8 v = 1;

	v = val ? 1 : 0;
	return bq2560x_update_bits(info, BQ2560X_REG_5,
				    BQ2560X_REG_ITERM_EN_MASK,
					v << BQ2560X_REG_ITERM_EN_SHIFT);
}
...
static int bq2560x_charger_hw_init(struct bq2560x_charger_info *info)
{
	...
	dev_warn(info->dev, "%s %d bat_id : %d.\n", __func__, __LINE__, bat_id);
	...
	if (ret) {
		...
		ret = bq2560x_charger_set_current(info,
							info->cur.sdp_cur);
		if (ret) {
			dev_err(info->dev, "set bq2560x chg current failed\n");
			return ret;
		}
	}
...
static int bq2560x_charger_get_vendor_id(struct bq2560x_charger_info *info)
{
	...
	if (ret < 0) {
		dev_err(info->dev, "read vendor id failed\n");
		return reg_val;
	}

	reg_val &= BQ2560X_VENDOR_ID_MASK;
	reg_val >>= BQ2560X_VENDOR_ID_SHIFT;
				  ...
				  u32 limit_cur)
{
	...
	switch (info->usb_phy->chg_type) {
		...
		case CDP_TYPE:
			...
			break;
		default:
			cur = info->cur.unknown_limit;
	}

	if (cur < limit_cur)
		limit_cur = cur;

	dev_err(info->dev, "%s set limit current = %d\n", __func__, limit_cur);
					    ...
					    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_ENABLE:
		val->intval = 0;
		break;

	case POWER_SUPPLY_PROP_ITERM:
		val->intval = 0;
		break;

	case POWER_SUPPLY_PROP_TIMER_ENABLE:
		val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_ITERM_ENABLE:
		val->intval = 0;
		break;
	...
	case POWER_SUPPLY_PROP_BATFET_DIS:
		val->intval = 0;
		break;

	case POWER_SUPPLY_PROP_TERM_STATUS:
		bq2560x_read(info, BQ2560X_REG_8, &term_v);
		val->intval = ((term_v & BIT(4)) && (term_v & BIT(3))) ? 1 : 0;
		break;
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_ENABLE:
		dev_info(info->dev, "%s :enable_charge = %d\n", __func__, val->intval);
		if (val->intval)
	...
	case POWER_SUPPLY_PROP_ITERM:
		if (info->role == BQ2560X_ROLE_MASTER_DEFAULT)
			ret = bq2560x_charger_set_termina_cur(info, 256);
		else
			ret = bq2560x_charger_set_termina_cur(info, 60);
	...
	case POWER_SUPPLY_PROP_ITERM_ENABLE:
		ret = bq2560x_charger_set_termina_en(info, val->intval);
		if (ret) {
			dev_err(info->dev, "set bq2560x terminal enable failed\n");
		}
		break;

	case POWER_SUPPLY_PROP_TIMER_ENABLE:
		set_en_timer_enable(0);
		dev_err(info->dev, "set bq2560x timer disable : %d\n", ret);
		break;

	case POWER_SUPPLY_PROP_VINDPM:
		ret = bq2560x_charger_set_vindpm(info, val->intval);
		if (ret) {
			dev_err(info->dev, "set bq2560x vindpm vol failed\n");
		}
		break;
	case POWER_SUPPLY_PROP_BATFET_DIS:
		ret = bq2560x_set_batfet_dis(info);
		if (ret) {
			dev_err(info->dev, "set bq2560x BATFET disable failed\n");
		}
		break;

	case POWER_SUPPLY_PROP_HIZ_STATUS:
		dev_err(info->dev, "%s set hiz status %d\n", __func__, __LINE__);
		ret = bq2560x_update_bits(info, BQ2560X_REG_0,
...
static enum power_supply_property bq2560x_usb_props[] = {
	...
	POWER_SUPPLY_PROP_IC_REG,
	POWER_SUPPLY_PROP_CHARGER_ENABLE,
		...
		const struct i2c_device_id *id)
{
	...
	info->chip_id = bq2560x_charger_get_vendor_id(info); 
	dev_err(dev, "%s chip_id with 0x%.2x\n", __func__, info->chip_id);
	g_charger_info = info;
	...
	/* disable kernel charge IC watchdog timer */
	bq2560x_set_watchdog_timer_status(info,0);
