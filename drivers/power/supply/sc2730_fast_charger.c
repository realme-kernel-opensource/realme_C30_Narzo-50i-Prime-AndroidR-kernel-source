				 ...
				 unsigned long event, void *data)
{
	...
	dev_info(info->dev, "fchg %s\n", __func__);
	if (strcmp(psy->desc->name, "bq2560x_charger") == 0) {
		dev_info(info->dev, "fchg %s ok\n", __func__);
		schedule_work(&info->qc_change_work);
		goto out;
	}
...
static void sc2730_fchg_qc_change_work(struct work_struct *data)
{
	struct sc2730_fchg_info *info =
		container_of(data, struct sc2730_fchg_info, qc_change_work);
	struct power_supply *psy;
	union power_supply_propval val;
	int ret;
	...
	ret = power_supply_get_property(psy,
				...
				&val);
	power_supply_put(psy);
	dev_info(info->dev, "fchg %s value = %d\n", __func__, val.intval);
...
static void sc2730_fchg_work(struct work_struct *data)
{
	...
	if (!info->limit) {
		...
		info->detected = true;
		if (info->pd_enable || info->qc_enable) {
			sc2730_fchg_disable(info);
			...
			mutex_unlock(&info->lock);
			cm_notify_event(info->psy_usb, CM_EVENT_FAST_CHARGE, NULL);
			dev_info(info->dev, "pd_enable = %d, sfcp_enable = %d, qc_enable = %d\n",
				info->pd_enable, info->sfcp_enable, info->qc_enable);
			return;
		} else {
			sc2730_fchg_disable(info);
		}
...
static int sc2730_fchg_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&info->qc_change_work, sc2730_fchg_qc_change_work);
