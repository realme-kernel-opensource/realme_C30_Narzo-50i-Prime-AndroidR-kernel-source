...
#define ATL_BATTERY_VOLTAGE_MIN_5000  	300
#define ATL_BATTERY_VOLTAGE_MAX_5000  	600
#define LWN_BATTERY_VOLTAGE_MAX_5000  	200
#define LWN_BATTERY_VOLTAGE_MIN_5000  	0
...
int sc27xx_fgu_get_bat_id_vol(struct sc27xx_fgu_data *data)
{
	...
	if (IS_ERR_OR_NULL(data->bat_id_cha)) {
		dev_err(data->dev, "bat-id iio channel is null\n");
		return ret;
	}
	...
	if ((id_vol >= ATL_BATTERY_VOLTAGE_MIN_5000)&&(id_vol <= ATL_BATTERY_VOLTAGE_MAX_5000)) {
		...
		sc27xx_fgu_bat_id = 1;
	} else if ((id_vol >= LWN_BATTERY_VOLTAGE_MIN_5000)&&(id_vol <= LWN_BATTERY_VOLTAGE_MAX_5000)) {
				   ...
				   int chg_sts)
{
	...
	*cap = delta_cap + data->init_cap;
	...
	dev_info(data->dev, "init_cap = %d, init_clbcnt = %d, cur_clbcnt = %d, normal_cap = %d, "
		"delta_cap = %d, Tbat  = %d, uusoc_vbat = %d, vol_1000mv_adc = %d, cur_1000ma_adc = %d\n",
		data->init_cap, data->init_clbcnt, cur_clbcnt,
		data->normal_temp_cap, delta_cap, data->bat_temp, data->uusoc_vbat,
		data->vol_1000mv_adc, data->cur_1000ma_adc);
	...
	total_mah = DIV_ROUND_CLOSEST(data->total_cap * temp_cap, 100);
	// if(chg_sts == POWER_SUPPLY_STATUS_DISCHARGING && avg_temp < 100 && avg_temp > 0) {
	// 	total_mah -= 2500;
	// 	printk("bms lt -%dmah\n", data->total_cap-total_mah);
	// }
	...
	*cap = delta_cap + data->init_cap;
	...
	dev_info(data->dev, "init_cap = %d, init_clbcnt = %d, cur_clbcnt = %d, normal_cap = %d, "
		 ...
		 "vol_1000mv_adc = %d, cur_1000ma_adc = %d, temp = %d\n",
		 data->init_cap, data->init_clbcnt, cur_clbcnt,
		 data->normal_temp_cap, delta_cap, data->bat_temp, data->uusoc_vbat,data->total_cap,total_mah,
		 data->vol_1000mv_adc, data->cur_1000ma_adc, temp);
			      ...
			      const struct sc27xx_fgu_variant_data *pdata)
{
	...
	if(ret){
		...
		dev_err(data->dev,"%s %d sc27xx_fgu_bat_id = %d.", __func__, __LINE__, sc27xx_fgu_bat_id);
	}
