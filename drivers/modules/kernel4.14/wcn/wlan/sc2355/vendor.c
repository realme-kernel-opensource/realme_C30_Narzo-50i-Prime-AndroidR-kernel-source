...
static struct set_sar_limit_param sar_param_map[] = {
	{
		...
		SPRDWl_SET_SAR_ABSOLUTE,
		9,
		SPRDWL_SET_SAR_ALL_MODE,
	},
	{
		/*recive off scence*/
						...
						const void *data, int len)
{
	...
	if (priv->extend_feature & SPRDWL_CAPA_TX_POWER) {
...
/**set sar param according to different scence**/
	...
	sar_scence = nla_get_u32(tb[WLAN_ATTR_SAR_LIMITS_SAR_ENABLE]);
	...
	if (psar_param) {
		...
		ret = sprdwl_set_sar(priv, vif->ctx_id,
						psar_param->sar_type,
						psar_param->power_value,
						psar_param->phy_mode);
		if (ret)
