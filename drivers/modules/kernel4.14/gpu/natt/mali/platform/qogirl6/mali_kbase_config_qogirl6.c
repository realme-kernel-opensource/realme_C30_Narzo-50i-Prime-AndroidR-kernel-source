...
{
	bool ret = false;
  	if((value & 0x7fc00000) == 0x7fc00000)
          ret = true;
        else
          ret = false;
	return ret;
...
int kbase_platform_set_freq_volt(int freq, int volt)
{
	...
	printk(KERN_INFO "mali GPU_DVFS %s index=%d cur_freq=%d cur_volt=%d --> freq=%d volt=%d gpu_power_on=%d gpu_clock_on=%d \n",
...
void kbase_platform_modify_target_freq(struct device *dev, unsigned long *target_freq)
{
	...
	printk(KERN_INFO "GPU_DVFS %s gpu_boost_level:%d min_freq=%d max_freq=%d target_freq=%lu \n",
