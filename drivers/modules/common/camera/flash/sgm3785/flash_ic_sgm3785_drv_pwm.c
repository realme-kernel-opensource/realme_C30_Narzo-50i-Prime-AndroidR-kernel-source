...
int flash_level[16] = {17, 23, 28, 34, 39, 45, 50, 56, 61, 67, 72, 78, 83, 89, 94, 100};

#define CAMERA_PREFLASH_LIGHT_DUTY 50
...
static int sprd_pin32_pwm_selected(void *drvd, bool is_pwm_selected)
{
    struct flash_driver_data *drv_data = (struct flash_driver_data *)drvd;
    int ret;
    if (is_pwm_selected) {
        ret =  pinctrl_select_state(drv_data->pin_gpio_32, drv_data->pinctrl_state_pwm);
    } else {
        ret =  pinctrl_select_state(drv_data->pin_gpio_32, drv_data->pinctrl_state_gpio);
    }
...
static int sprd_flash_sgm3785_open_torch(void *drvd, uint8_t idx)
{
    ...
    if (SPRD_FLASH_LED0 & idx) {
        ...
        if(drv_data->is_old_board){
            ...
            if (gpio_is_valid(drv_data->gpio_32_ENM)){
                gpio_direction_output(drv_data->gpio_32_ENM, SPRD_FLASH_ON);
                udelay(5 * 1000);
                gpio_direction_output(drv_data->gpio_32_ENM, SPRD_FLASH_OFF);
            }
            sprd_pin32_pwm_selected(drv_data, true);
...
static int sprd_flash_sgm3785_close_torch(void *drvd, uint8_t idx)
{
    ...
    if (SPRD_FLASH_LED0 & idx) {
        ...
        pwm_set_sgm3785_config(drv_data->pwm_chip, 0);
        sprd_pin32_pwm_selected(drv_data, false);
        if (gpio_is_valid(drv_data->gpio_32_ENM)){
            gpio_direction_output(drv_data->gpio_32_ENM, SPRD_FLASH_OFF);
        }
        udelay(5 * 1000);
...
static int sprd_flash_sgm3785_cfg_value_highlight(void *drvd, uint8_t idx,
    ...
    if (SPRD_FLASH_LED0 & idx){
        ...
        drv_data->torch_led_index = (int) flash_level[level];
        pr_info("level:%d,torch_led_index:%d", level, drv_data->torch_led_index);
    }
...
static int sprd_flash_sgm3785_probe(struct platform_device *pdev)
{
    ...
    drv_data->gpio_32_ENM = of_get_named_gpio(pdev->dev.of_node, FLASH_GPIO_ENM_NAME, 0);
    if (gpio_is_valid(drv_data->gpio_32_ENM)) {
        ret = devm_gpio_request(&pdev->dev,
                                drv_data->gpio_32_ENM, FLASH_GPIO_ENM_NAME);
        pr_err("gpio init gpio %d\n",drv_data->gpio_32_ENM);
        if (ret) {
            pr_err("flash gpio err\n");
            goto exit;
        }
    ...
    ret = gpio_direction_output(drv_data->gpio_32_ENM, SPRD_FLASH_OFF);
    if (ret) {
        pr_err("flash gpio output err\n");
        goto exit;
    }

    drv_data->pin_gpio_32 = devm_pinctrl_get(&pdev->dev);
    drv_data->pinctrl_state_gpio = pinctrl_lookup_state(drv_data->pin_gpio_32, "gpio_32");
    drv_data->pinctrl_state_pwm = pinctrl_lookup_state(drv_data->pin_gpio_32, "rfctl13_32_pwm");
