...
static int8_t g_sar_init_complete = 0;
...
static void hx9031a_ch_cfg(uint8_t chip_select)
{
    ...
    if(HX9023E_ON_BOARD == chip_select) {
        ...
        CH1_NEG = NA;
        CH2_POS = CS4;  //grip_sensor_wifi
        CH2_NEG = CS3;  //grip_sensor_wifi_ref
        CH3_POS = NA;
        CH3_NEG = NA;
...
static inline void sar_report_distance(struct input_dev *dev,unsigned int code, int value )
{
    ...
    if(g_sensor && value >= 0){
        ...
        if(!strncmp(dev->name,"grip_sensor_wifi",sizeof("grip_sensor_wifi")))
            event.HandleID = 83;
        event.udata[2] = value;
        LOG_INFO("event.udata[2]:%d\n",event.udata[2]);
        get_monotonic_boottime(&tp);
        event.timestamp =timespec_to_ns(&tp);
        ...
        if (hx9031a_channels[ii].used == true){
            if (strcmp(sensors_cdev->name, hx9031a_channels[ii].hx9031a_sensors_classdev.name) == 0) {
                if (1 == enable) {
                    ...
                    sar_report_distance(hx9031a_channels[ii].hx9031a_input_dev, ABS_DISTANCE, 5);
...
//fanwenming for shub_core.c use
    ...
    switch (id){
        case 81:
            if (enable == 1){
                ...
                LOG_INFO("name:grip_sensor:distance\n");
        ...
        case 82:
            if (enable == 1){
                ...
                LOG_INFO("name:grip_sensor_sub:distance\n");
        ...
        case 83:
            if (enable == 1){
                ...
                LOG_INFO("name:grip_sensor_wifi:distance\n");
...
/*---input_dev attrabute enable for realme hal,fanweenming.wt*/
...
static ssize_t ch0_cap_diff_dump_show(struct class *class,struct class_attribute *attr,char *buf)
{
    ssize_t len = 0;
    uint8_t rx_buf[2] = {0};
    uint32_t cap_reg = 0;
    ...
    cap_reg = cap_reg & 0xFFF; //12位
    PRINT_ERR("CH0_background_cap=%d\n", cap_reg);
    len += snprintf(buf+len,PAGE_SIZE-len,"CH0_background_cap=%d;", cap_reg );
    ...
    len += snprintf(buf+len,PAGE_SIZE-len,"CH0_refer_channel_cap=%d;", cap_reg );

    //CH0 diff
    hx9031a_sample();
    len += snprintf(buf+len,PAGE_SIZE-len,"CH0_diff=%d",data_diff[0]);

    return (len);//return numbers of parameters
}
static CLASS_ATTR_RO(ch0_cap_diff_dump);

static ssize_t ch2_cap_diff_dump_show(struct class *class,struct class_attribute *attr,char *buf)
{
    ssize_t len = 0;
    uint8_t rx_buf[2] = {0};
    uint32_t cap_reg = 0;
    ...
    len += snprintf(buf+len,PAGE_SIZE-len,"CH2_background_cap=%d;", cap_reg );
    ...
    len += snprintf(buf+len,PAGE_SIZE-len,"CH2_refer_channel_cap=%d;", cap_reg );

    //CH2 diff
    hx9031a_sample();
    len += snprintf(buf+len,PAGE_SIZE-len,"CH2_diff=%d",data_diff[1]);
    return (len);//return numbers of parameters
}
static CLASS_ATTR_RO(ch2_cap_diff_dump);

static ssize_t ch3_cap_diff_dump_show(struct class *class,struct class_attribute *attr,char *buf)
{
    ssize_t len = 0;
    uint8_t rx_buf[2] = {0};
    uint32_t cap_reg = 0;
    ...
    len += snprintf(buf+len,PAGE_SIZE-len,"CH3_background_cap=%d;", cap_reg );
    ...
    len += snprintf(buf+len,PAGE_SIZE-len,"CH3_refer_channel_cap=%d;", cap_reg );

    //CH3 diff
    hx9031a_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH3_diff=%d",data_diff[2]);
    return (len);//return unmbers of parameters
}
...
//}end =============================================================
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if ((hx9031a_pdata.channel_used_flag >> ii) & 0x1) { //used flag的值来自dts，表示通道有效性，每一位表示一个通道。
            ...
            hx9031a_channels[ii].hx9031a_sensors_classdev.type = SENSOR_TYPE_TYHX_CAPSENSE;
    ...
    if (ret < 0){
        PRINT_INF("register capsense class failed (%d)\n", &capsense_class);
        return ret;
    }
    ...
    hardwareinfo_set_prop(HARDWARE_SAR,"HX9031A,FW:V0.1");
    g_sar_init_complete = 1;
