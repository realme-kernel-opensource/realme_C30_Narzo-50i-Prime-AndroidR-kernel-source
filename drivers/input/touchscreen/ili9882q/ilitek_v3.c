...
static void ilitek_resume_by_ddi_work(struct work_struct *work)
{
	...
	case TOUCHSCREEN_SUSPEND:
		ILI_INFO("TP suspend start by wq\n");
		...
		if (sense_stop) {
			...
			if ((ilits->gesture) || (ilits->prox_face_mode == true) || tp_gesture) {
			...
			ILI_INFO("TP suspend end by wq\n");
			break;
	case TOUCHSCREEN_RESUME:
		ILI_INFO("TP resume start by wq\n");
...
static void ilitek_tddi_wq_init(void)
{
	...
	ilits->notifier_suspend_resume.notifier_call = ili_pannel_notify_callback;
	res = tp_pannel_register_client(&ilits->notifier_suspend_resume);
#endif
...
int ili_report_handler(void)
{
	...
	if (ret < 0) {
		...
		if (ret == DO_SPI_RECOVER) {
			...
			if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE && ilits->gesture  && tp_gesture&& !ilits->prox_near) {
...
int ili_tddi_init(void)
{
	...
	ilits->chip->devinfo_tp_has_init = false;
