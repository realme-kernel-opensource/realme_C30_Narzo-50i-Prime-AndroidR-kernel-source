...
int parse_black_selftest_dt(struct cts_oem_data * oem_data, struct device_node * np)
{
    ...
    if (oem_data->test_gesture_rawdata) {
       ...
       /* ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES,
		...
		cts_info("OEM_OF_DEF_PROPVAL_TEST_GESTURE_RAWDATA");
...
int auo_parse_black_selftest_dt(struct cts_oem_data * oem_data, struct device_node * np)
{
	...
	oem_data->test_gesture_rawdata = OEM_OF_DEF_PROPVAL_TEST_GESTURE_RAWDATA;
    if (oem_data->test_gesture_rawdata) {
        oem_data->gesture_rawdata_test_frames = AUO_OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_FRAMES;
       /* ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES,
            &oem_data->gesture_rawdata_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_FRAMES"' failed %d", ret);
        ...
        oem_data->gesture_rawdata_min = AUO_OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_MIN;
       /* ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_MIN,
            &oem_data->gesture_rawdata_min);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MIN"' failed %d", ret);
        ...
        oem_data->gesture_rawdata_max = AUO_OEM_OF_DEF_PROPVAL_GESTURE_RAWDATA_MAX;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_RAWDATA_MAX,
            (u32 *)&oem_data->gesture_rawdata_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_RAWDATA_MAX"' failed %d", ret);
	...
	oem_data->test_gesture_lp_rawdata = OEM_OF_DEF_PROPVAL_TEST_GESTURE_LP_RAWDATA;
    if (oem_data->test_gesture_lp_rawdata) {
        oem_data->gesture_lp_rawdata_test_frames = AUO_OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_FRAMES;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_FRAMES,
            &oem_data->gesture_lp_rawdata_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_FRAMES"' failed %d", ret);
        ...
        oem_data->gesture_lp_rawdata_min = AUO_OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_MIN;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MIN,
            &oem_data->gesture_lp_rawdata_min);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MIN"' failed %d", ret);
        ...
        oem_data->gesture_lp_rawdata_max = AUO_OEM_OF_DEF_PROPVAL_GESTURE_LP_RAWDATA_MAX;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MAX,
            (u32 *)&oem_data->gesture_lp_rawdata_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_RAWDATA_MAX"' failed %d", ret);
	...
	oem_data->test_gesture_noise = OEM_OF_DEF_PROPVAL_TEST_GESTURE_NOISE;
    if (oem_data->test_gesture_noise) {
        oem_data->gesture_noise_test_frames = AUO_OEM_OF_DEF_PROPVAL_GESTURE_NOISE_FRAMES;
       /* ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_NOISE_FRAMES,
            &oem_data->gesture_noise_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_NOISE_FRAMES"' failed %d", ret);
        ...
        oem_data->gesture_noise_max = AUO_OEM_OF_DEF_PROPVAL_GESTURE_NOISE_MAX;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_NOISE_MAX,
            &oem_data->gesture_noise_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_NOISE_MAX"' failed %d", ret);
	...
	oem_data->test_gesture_lp_noise = OEM_OF_DEF_PROPVAL_TEST_GESTURE_LP_NOISE;
    if (oem_data->test_gesture_lp_noise) {
        oem_data->gesture_lp_noise_test_frames = AUO_OEM_OF_DEF_PROPVAL_GESTURE_LP_NOISE_FRAMES;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_NOISE_FRAMES,
            &oem_data->gesture_lp_noise_test_frames);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_FRAMES"' failed %d", ret);
        ...
        oem_data->gesture_lp_noise_max = AUO_OEM_OF_DEF_PROPVAL_GESTURE_LP_NOISE_MAX;
        /*ret = of_property_read_u32(np, OEM_OF_PROPNAME_GESTURE_LP_NOISE_MAX,
            &oem_data->gesture_lp_noise_max);
        if (ret) {
            cts_warn("Parse '"OEM_OF_PROPNAME_GESTURE_LP_NOISE_MAX"' failed %d", ret);
    ...
    struct cts_test_param *param)
{
    ...
    if (cts_dev == NULL || param == NULL ||
        param->priv_param_size != sizeof(*priv_param) ||
        param->priv_param == NULL) {
        cts_err("Gesture rawdata test with invalid priv param fail\n");
        return -EINVAL;
    }

    priv_param = param->priv_param;
    if (priv_param->frames <= 0) {
        cts_info("Gesture rawdata test with too little frame fail\n");
        return -EINVAL;
    }
        ...
        struct cts_test_param *param)
{
    ...
    if (cts_dev == NULL || param == NULL ||
        param->priv_param_size != sizeof(*priv_param) ||
        param->priv_param == NULL) {
        cts_err("Noise test with invalid param: priv param fail\n");
        return -EINVAL;
    }

    priv_param = param->priv_param;
    if (priv_param->frames < 2) {
        cts_err("Noise test with too little frame fail\n");
        return -EINVAL;
    }
