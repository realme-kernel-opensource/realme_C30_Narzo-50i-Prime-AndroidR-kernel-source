...
extern int aw87xxx_power_control(bool enable);
extern void sia81xx_power_control(bool enable);
			       ...
			       struct snd_kcontrol *k, int event)
{
	...
	if(pa_select == 1){
		if(on)
			sia81xx_power_control(true);
		else
			sia81xx_power_control(false);
	}else{
		if(on)
			aw87xxx_power_control(true);
		else
			aw87xxx_power_control(false);
	}

	/*wait for hw init complete,than pcm start*/
	msleep(10);
...
extern int sprd_sia81xx_mode_put(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);
extern int sprd_sia81xx_mode_get(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);
...
static const struct snd_kcontrol_new _sprd_asoc_card_controls[] = {
    ...
    SOC_SINGLE_EXT("Sia81xx Mode", 0, 0, INT_MAX, 0,
    sprd_sia81xx_mode_get, sprd_sia81xx_mode_put),
};
