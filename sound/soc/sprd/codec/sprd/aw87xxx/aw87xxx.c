...
extern void hook_gpio_off_set(void);
extern void hook_gpio_request_set(void);
extern int pa_select;
			...
			struct snd_ctl_elem_info *uinfo)
{
 ...
 void set_audio_secne_mode(int mode_sence_hal)
{
            mode_pa =  mode_sence_hal;
}
...
int aw87xxx_power_control(bool enable)
{
	struct list_head *pos = NULL;
	struct aw87xxx *aw87xxx = NULL;
	char *profile_name = NULL;
	...
	list_for_each(pos, &g_aw87xxx_list) {
		aw87xxx = list_entry(pos, struct aw87xxx, list);
		 if (aw87xxx == NULL) {
			 AW_LOGE("%s,struct aw87xxx not ready",__func__);
			 return ret;
		 }
	 }

	profile_name = aw87xxx_acf_get_prof_name_form_index(aw87xxx->dev, &aw87xxx->acf_info, mode_pa);

	if (!profile_name) {
		AW_LOGE("%s,profile name not found,mode_pa=[%d]",__func__,
				mode_pa);
		return -EINVAL;
	}

	AW_DEV_LOGI(aw87xxx->dev, "aw_enable:%d,mode_pa:%d,profile is [%s]",enable,mode_pa,profile_name);
	...
	if (ret < 0) {
		AW_LOGE("%s,set dev_index[%d] profile failed, profile = [%s]",
			__func__,aw87xxx->dev_index, profile_name);
		return ret;
	}

	return ret;
}
EXPORT_SYMBOL(aw87xxx_power_control);
...
exit_device_init_failed:
	...
	pa_select = 1;  /*select pa is sia81xxx*/
	hook_gpio_request_set();
	hook_gpio_off_set();
	aw87xxx = NULL;
